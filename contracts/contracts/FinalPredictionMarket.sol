...
contract FinalPredictionMarket {
  ...
  mapping(address => uint256) public userMarketStakeCount;
  ...
  function placeStake(uint256 _marketId, bool _prediction) external payable {
    ...
    if (market.userStake[msg.sender] == 0) {
      userMarketStakeCount[msg.sender]++;
    }
  ...
  function hasUserStakedInMarket(uint256 _marketId, address _user) external view returns (bool hasStaked, uint256 stakedAmount) {
    require(_marketId > 0 && _marketId <= marketCount, "Invalid market ID");
    Market storage market = markets[_marketId];
    return (market.userStake[_user] > 0, market.userStake[_user]);
  }
}